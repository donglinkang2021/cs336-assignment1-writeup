...
\begin{enumerate}[label=(\alph*)]
    ...
    \begin{answer}
    \textbf{Parameters}: (See Appendix~\ref{sec:param-verification} for verification of this formula)
    \begin{equation*}
    ...
    (where $V$ = \lstinline{vocab_size}, $d$ = \lstinline{d_model}, $L$ = \lstinline{num_layers}, $T$ = \lstinline{context_length}, $H$ = \lstinline{num_heads}, $B$ = \lstinline{batch_size}, $d_{ff} = 4d$, and $d_v = d / H$)
    \end{answer}
    ...
    Parameters: $P = 16 \times 48 \times 1600^2 + (2 \times 48 + 1 + 2 \times 50257) \times 1600 \approx 1.98\text{B parameters}$
    ...
    \begin{answer}
    % AdamW requires approximately $6P$ FLOPs per step, where $P$ is the number of parameters. This includes: momentum updates (2P), variance updates (3P), and parameter updates (1P). The dominant operations are element-wise multiplications and additions across all parameters.
    \end{answer}
    ...
    \begin{answer}
    % Forward pass: 4.51T FLOPs, Backward pass: 9.02T FLOPs, Total per step: 13.53T FLOPs

    % With 50\% MFU on A100: $19.5 \times 0.5 = 9.75$ teraFLOP/s effective throughput

    % Time per step: $\frac{13.53}{9.75} = 1.39$ seconds

    % Total training time: $400,000 \times 1.39 = 556,000$ seconds $\approx 154$ hours $\approx 6.4$ days

    % This assumes perfect scaling and no overhead from data loading, checkpointing, or other operations.
    \end{answer}
\end{enumerate}